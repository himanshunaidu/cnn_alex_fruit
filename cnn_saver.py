...
#Take in RGB images
convo1 = convolutional_layer(x_image, shape=[11, 11, 3, 96], stridec=[1, 4, 4, 1])
#lrn1 = LRN(convo1, 2, 2e-05, 0.75, "norm1")
convo1_pool = max_pool_3by3(convo1)

convo2 = convolutional_layer(convo1_pool, shape=[5, 5, 96, 256])
#lrn2 = LRN(convo2, 2, 2e-05, 0.75, "norm1")
convo2_pool = max_pool_3by3(convo2)

convo3 = convolutional_layer(convo2_pool, shape=[5, 5, 256, 384])

convo4 = convolutional_layer(convo3, shape=[3, 3, 384, 384])

convo5 = convolutional_layer(convo4, shape=[3, 3, 384, 256])
...
step_saver = open('<PATH>/TempF4/steps.txt', 'a+')
acc_saver = open('<PATH>/TempF4/acc.txt', 'a+')
...
with tf.Session() as sess:
    ...
    for i in range((actual_steps-runs), steps):
        ...
        train_time_saver = open('<PATH>/TempF4/train_time.txt', 'a+')
